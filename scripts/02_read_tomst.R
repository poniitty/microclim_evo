...
mind <- as.Date("2020-01-01", tz = "Etc/GMT-2")
maxd <- as.Date("2022-11-24", tz = "Etc/GMT-2")

raw_data_dir <- "E:/evo2022"
...
readdata <- function(i){
  ...
  if(nn > 1){
    ...
    print(i)
    d <- fread(fi$file[fi$file2 == i], header = FALSE)
...
office <- c(as_date(as_date(min(df$datetime)):as_date("2021-10-25")))
probls <- c()
hattu <- c()
T2T3error <- c()
...
office <- c(as_date(as_date(min(df$datetime)):as_date("2021-10-24")))
probls <- c()
hattu <- c()
T2T3error <- c()
...
probls <- c()
hattu <- c()
T2T3error <- c(as_date(as_date("2021-10-26"):as_date("2022-11-22")))
...
probls <- c()
hattu <- c()
T2T3error <- c(as_date(as_date("2021-10-25"):as_date("2022-11-22")))
...
for(i in sites){
  ...
  df3 %>% mutate(T1 = ifelse(site == i,T1 - diffs["T1"],T1),
                 T2 = ifelse(site == i,T2 - diffs["T2"],T2),
                 T3 = ifelse(site == i,T3 - diffs["T3"],T3)) -> df3
...
for(i in sites){
  ...
  print(i)
  dfall %>% filter(site == i) %>% 
    mutate(T1 = ifelse(probl %in% c(1,4,9), NA, T1)) %>% 
