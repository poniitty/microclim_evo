...
raw_data_dir <- "E:/evo2022/HOBO"

mind <- as_date("2021-10-25", tz = "Etc/GMT-2")

#################################################################
# List logger data files to read

f <- list.files(raw_data_dir, pattern = ".csv$", full.names = T)

fi <- data.frame(file = f)

fi$site <- as.numeric(parse_number(unlist(lapply(fi$file, function(x) rev(strsplit(x, "/")[[1]])[1]))))
...
df <- df %>% 
  filter(datetime > mind)

sites <- unique(df$site)

df %>% mutate(roundtime = round_date(datetime, "60 minutes")) -> df
...
fwrite(df7, "output/hobo_data_corrected.csv")
